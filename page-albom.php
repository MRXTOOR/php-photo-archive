...
if (isset($_SESSION['user_name'])) {
    ...
    // Ваш код для отображения имени пользователя
    echo '<p class="user-info">Пользователь: ' . $user_name . '</p>';

    // Далее ваш код для отображения альбома и фотографий
} else {
    // Если имя пользователя не найдено в сессии, можно выполнить какое-то действие, например, перенаправить на страницу авторизации
    header("Location: login.php");
}
// Проверяем, существует ли параметр album_id в URL
if(isset($_GET['album_id'])) {
    // Получаем ID_Album из URL
    ...
    if ($albumResult->num_rows > 0) {
        ...
        while ($photoRow = $photosResult->fetch_assoc()) {
            ...
            echo '<img src="' . $photoRow['FilePath'] . '" alt="Фотография">';
            echo '</a>';
        }
